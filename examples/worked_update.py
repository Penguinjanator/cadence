"""One free/nudged update on a six-neuron net, every number printed (docs/learning.md, section 4).
...
connectome = cd.layered(2, 2, 2, density=1.0, seed=3)
...
w = connectome
p = w.populations
print("neurons: inputs", p["input"], "hidden", p["hidden"], "outputs", p["output"])
print("synapses (pre -> post: scale):")
for e in range(w.synapses):
    print(f"  {w.pre[e]} -> {w.post[e]}: {learner.brain.efficacy[e]:+.3f}")

drive = learner.brain.stimulus_levels(np.pad([[1.0, 0.2]], ((0, 0), (0, w.n - 2))))
label = np.array([1])
...
synapse_term, neuron_term = learner.contrast(free, plus, minus)
before = learner.brain.efficacy.copy()
learner.update(free, plus, minus)
print("\ncontrast per synapse and the resulting scale (eta 1, tied pairs averaged):")
for e in range(w.synapses):
    now = learner.brain.efficacy[e]
    synapse = f"{w.pre[e]} -> {w.post[e]}"
    print(f"  {synapse}: contrast {synapse_term[e]:+.4f}   scale {before[e]:+.3f} -> {now:+.4f}")
print("bias change per neuron:", np.round(config.eta_bias * neuron_term, 5))
