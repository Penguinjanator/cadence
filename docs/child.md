...
| plasticity only for a positive or negative signal, in proportion | the dopamine centred on its running level per stream, in the reward's own units, nothing within the usual, capped (`dopamine_center`, `center_scale=False`, `dopamine_floor`, `dopamine_cap`; per stream always), no critic | after thirty moments of a reward of one: the same reward again, a missing reward, a reward ten larger | seam change **0.000**, 0.082, 0.279 |

| what is still ringing is what gets written | the eligibility of each seam weighted by its pre owner's ringing (`ActorCritic.salience` from `Afterglow.ringing`) | a cue seen for one moment with its press, paid three moments later, twelve owners of background always on | plain trace 1.00, weighted 1.00 at a hundred rounds; at fifty rounds 0.57 against 1.00 on one seed and no difference on another |
...
the components in the core, no critic needed. The coupling of the two, the eligibility
written through what still rings, is built and correct and, in the small experiment, not
yet a gain: the background's eligibility is noise that averages out over rounds, so the
plain trace finds the cue as well, and the weighting only sometimes finds it sooner. It
stays off unless set; the player is where it can show a difference, on a retina of seven
thousand owners where most of the picture stands still.
...
3. The coupling in the player: the eligibility weighted by the afterimage's ringing during
   self-play, where the small experiment could not tell it from the plain trace.
