...
def test_activity_change_is_the_path_length_of_the_activations_and_agrees_across_paths() -> None:
    ...
    assert fused.activity_change is not None and loop.activity_change is not None and loop.trajectory is not None
    ...
    assert single.activity_change is not None and np.isclose(float(single.activity_change), fused.activity_change[0])
...
def test_torch_reports_the_same_activity_change() -> None:
    ...
    neuron_model = cd.learning_neuron_model(dt=1.0)
    drive = cd.Brain(w, neuron_model).stimulus_levels(np.random.default_rng(1).random((2, w.n)) * 0.5)
