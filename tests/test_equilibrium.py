...
def isolated(rule: cd.GradedRule) -> cd.Settlement:
    return cd.Settlement(cd.Wiring.from_edges(1, pre=[], post=[]), rule)


def test_activation_tolerance_can_stop_far_from_equilibrium() -> None:
    engine = isolated(cd.learning_rule(dt=0.1))
    drive = np.array([100.0])
    stopped = engine.settle(drive, steps=300, tolerance=1e-8)
    assert stopped.steps < 10  # the activation saturates while potential still moves
    assert engine.residual(drive, stopped)[0] > 60
    settled = engine.settle(drive, steps=300)
    assert engine.residual(drive, settled)[0] < 1e-10
    slower = isolated(cd.learning_rule(dt=0.001))
    np.testing.assert_allclose(slower.residual(drive, stopped), engine.residual(drive, stopped))


def test_residual_includes_slow_adaptation_equation() -> None:
    rule = cd.learning_rule().replace(adaptation=cd.Adaptation(tau_steps=1e6, strength=0.5))
    engine = isolated(rule)
    state = cd.SettledState(np.ones(1), rule.activation(np.ones(1)), np.zeros(1), 0)
    # The potential equation already holds, but the adaptation is far from its limit.
    assert engine.residual(np.ones(1), state)[0] > 0.4
    a = state.activation.copy()
    equilibrium = cd.SettledState(state.v, state.activation, a, 0)
    assert engine.residual(np.ones(1) + 0.5 * a, equilibrium)[0] < 1e-12


def test_rule_replace_preserves_adaptation_object() -> None:
    adaptation = cd.Adaptation(tau_steps=20, strength=0.3)
    original = cd.learning_rule().replace(adaptation=adaptation)
...
def test_residual_matches_the_projected_owner_equation_without_mutation(
    dense_limit: int, row_masks: bool
) -> None:
    wiring = cd.Wiring.from_edges(
        3, pre=[0, 1, 2], post=[1, 2, 0], count=[2, 3, 4], sign=[0.2, -0.1, 0.3]
    )
    rule = cd.learning_rule(gain=0.7, dt=0.3).replace(
    ...
    state = cd.SettledState(v.copy(), rule.activation(v) * keep, a.copy(), 0)
    ...
    following = engine.settle_batch(
        drive, steps=1, state=state, mask=keep, nudge=nudge, trajectory=True
    )
    expected = np.maximum(
        np.abs((following.v - v) / rule.dt).max(axis=1),
        np.abs(rule.activation(v) * keep - a).max(axis=1),
    )
    np.testing.assert_allclose(
        engine.residual(drive, state, nudge=nudge, mask=keep), expected, atol=1e-12
    )
    np.testing.assert_array_equal(state.v, v)
    np.testing.assert_array_equal(state.adaptation, a)
    assert engine.residual(drive, state).shape == (2,)


def test_ablation_requires_zero_potential_and_adaptation() -> None:
    rule = cd.learning_rule().replace(adaptation=cd.Adaptation())
    engine = isolated(rule)
    state = cd.SettledState(np.array([1.0]), np.zeros(1), np.array([0.4]), 0)
    assert engine.residual(np.array([100.0]), state, mask=np.zeros(1))[0] >= 1.0
    dead = cd.SettledState(np.zeros(1), np.zeros(1), np.zeros(1), 0)
    assert engine.residual(np.array([100.0]), dead, mask=np.zeros(1))[0] == 0.0


def test_residual_does_not_claim_a_unique_equilibrium() -> None:
    wiring = cd.Wiring.from_edges(2, pre=[0, 1], post=[1, 0], sign=[6, 6])
    rule = cd.GradedRule(gain=1.0, dt=0.5)
    engine = cd.Settlement(wiring, rule)
    drive = np.zeros(2)
    cold = engine.settle(drive, steps=200)
    initial = cd.SettledState(np.full(2, 6.0), rule.activation(np.full(2, 6.0)), np.zeros(2), 0)
    warm = engine.settle(drive, steps=200, state=initial)
    assert engine.residual(drive, cold)[0] < 1e-12
    assert engine.residual(drive, warm)[0] < 1e-12
...
def test_residual_rejects_wrong_shapes_and_does_not_certify_nan() -> None:
    engine = isolated(cd.learning_rule())
    state = engine.settle(np.zeros(1), steps=1)
    with pytest.raises(ValueError, match="column"):
        engine.residual(np.zeros(2), state)
    with pytest.raises(ValueError, match="batch"):
        engine.residual(np.zeros((2, 1)), state)
    with pytest.raises(ValueError, match="mask"):
        engine.residual(np.zeros(1), state, mask=np.zeros(2))
    invalid = cd.SettledState(np.array([np.nan]), np.array([np.nan]), np.zeros(1), 0)
    assert np.isinf(engine.residual(np.zeros(1), invalid)[0])
...
def test_raw_contrast_needs_parameter_and_loss_units_for_exact_gradient(nudge: str) -> None:
    ...
    all owners remain on a smooth activation branch. Each perturbed parameter
    moves both directions of one physical seam, so one contrast enters.
    """
    wiring = cd.Wiring.from_edges(
        3,
        pre=[0, 1, 0, 2, 1, 2],
    ...
    learner = cd.Learner(engine, [1, 2], config)
    ...
    assert engine.residual(drive, free).max() < 1e-11
    assert engine.residual(drive, plus, nudge=learner.nudge_for(target, config.beta)).max() < 1e-11
    assert (
        engine.residual(drive, minus, nudge=learner.nudge_for(target, -config.beta)).max() < 1e-11
    )

    def loss(candidate: cd.Settlement) -> float:
    ...
    for i, j in ((0, 1), (0, 2), (1, 2)):
        pair = ((wiring.pre == i) & (wiring.post == j)) | ((wiring.pre == j) & (wiring.post == i))
        edge = np.flatnonzero(pair)[0]
        up, down = engine.edge_scale.copy(), engine.edge_scale.copy()
        up[pair] += 1e-5
        down[pair] -= 1e-5
        negative_gradient = (
            -(
                loss(engine.with_parameters(edge_scale=up))
                - loss(engine.with_parameters(edge_scale=down))
            )
            / 2e-5
        )
        parameter_units = (
            engine.rule.gain * wiring.count[edge] * np.exp(engine.log_gain[wiring.pre[edge]])
        )
