...
def test_act_reuses_the_phase_learn_settled_and_a_new_batch_size_drops_the_traces() -> None:
    c, _, _ = _setup()
    ...
    settled = agent.state
    agent.act(d1)
    assert agent.state is settled  # the phase learn settled serves the next decision
    agent.act(_drive(c, rng, 2))
